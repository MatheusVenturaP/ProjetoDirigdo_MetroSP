...
def analisar_fluxo_total_remanescente(historico_de_perturbacoes):
    ...
    resiliencia_falhas_aleatorias = sum(eixo_y1[1:]) / (len(eixo_y1) - 1)
    ...
    resiliencia_ataques_coordenados_grau = sum(eixo_y2[1:]) / (len(eixo_y2) - 1)
    ...
    resiliencia_ataques_coordenados_betweeness = sum(eixo_y3[1:]) / (len(eixo_y3) - 1)
    ...
    plt.plot(eixo_x1, eixo_y1, label="Falhas Aleatórias (" + str("{:.4f}".format(resiliencia_falhas_aleatorias)) + ")")
    plt.plot(eixo_x2, eixo_y2, label="Ataque por grau (" + str("{:.4f}".format(resiliencia_ataques_coordenados_grau)) +
                                     ")")
    plt.plot(eixo_x3, eixo_y3, label="Ataque por betweenness (" + str("{:.4f}".format(
        resiliencia_ataques_coordenados_betweeness)) + ")")
...
def analise_de_resiliencia(g, modal):
...
def executa_processo(arquivo):
    ...
    while path < 2:
        ...
        analise_de_resiliencia(g, modal=modal)
        path += 1